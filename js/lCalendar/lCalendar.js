	...
	MobileCalendar.prototype = {
		...
		bindEvent: function(type) {
			...
			function popupDate(e) {
				...
				if(_self.trigger.value){
					_self.setTime = new Date(_self.trigger.value);
				}else{
					_self.setTime = new Date();
				}
			...
			function finishMobileDate(e) {
				...
				_self.okCallback && _self.okCallback(_self.setTime,new Date(_self.trigger.value),_self.trigger);
			...
			function finishMobileDateTime(e) {
				...
				_self.trigger.value = (date_yy % passY + _self.minY) + "/" + date_mm + "/" + date_dd + " " + (time_hh.length < 2 ? "0" : "") + time_hh + (time_mm.length < 2 ? ":0" : ":") + time_mm+":00";
				_self.okCallback && _self.okCallback(_self.setTime,new Date(_self.trigger.value),_self.trigger);
				closeMobileCalendar(e);
			}
			//时间确认
