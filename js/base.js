...
var resizeFont=function(){
	var _FONT_SIZE = (window.screen.width / 3.75/2);
	var maxFontSize =window.MAX_FONT_SIZE||75;
	window._FONT_SIZE =_FONT_SIZE>maxFontSize?maxFontSize:(_FONT_SIZE<40?40:_FONT_SIZE);
	(document.childNodes[1] || document.children[0]).style.fontSize = _FONT_SIZE + "px";
};
resizeFont();

//屏幕旋转
window.addEventListener("onorientationchange" in window ? "orientationchange" : "resize", resizeFont, false);

var worf = {};

//@if TARGETS='pro'
 worf.debug =false;
 worf.debugData =false;
 worf.resMode="pro";
//@endif
...
(function ($) {
    ...
    //@if TARGETS='pro'
		worf.API_URL = 'http://csh.lewei666.com';
		worf.origin = "http://csh.lewei666.com/H5";
		//@endif
	
		//@if TARGETS='uat'
		worf.API_URL = 'http://172.16.88.216:8082';
		worf.origin = "http://172.16.88.216:8082/H5";
		//@endif
	  ...
	  worf.API_URL = 'http://127.0.0.1:8080';
	  worf.origin = document.location.origin;
	  //@endif
	  
	  if(worf.origin=='http://106.12.86.100'){
	  	worf.API_URL = 'http://106.12.86.100:8080';
	  	worf.origin = document.location.origin+'/myWork/wx-project/cheshenghuo-wx';
	  }
  ...
  **/
    ...
    clear: function () {
      ...
      function existKey(key) {
        for (var i = 0; i < ignoreKeys.length; i++) {
          if (new RegExp(ignoreKeys[i], "ig").test(key)) {
            return true;
          }
        }
        return false;
      ...
      for (var key in localStorage) {
        if (localStorage.hasOwnProperty(key) && !existKey(key)) {
   ...
   * 设备识别
   */
  worf.device = (function () {
    var deviceType = (worf.cookie.get("source") || "wap").toLowerCase();
    return {
      ios: (deviceType == "ios"),
  ...
  * */
  worf.nav = {
    ...
    */
     ...
     * @wrapper 外层容器是谁
     */
    changeHeader: function (showIndex,wrapper) {
    	if(typeof(showIndex)=="string"){
    		wrapper=showIndex;
    		showIndex=undefined;
    	}
      if(showIndex!=null && showIndex!=undefined){
      	$(".back").attr("data-show", "false").eq(showIndex).attr("data-show", "true");
        $(".header_right").attr("data-show", "false").eq(showIndex).attr("data-show", "true");
      }else{
				$(".back,.header_right").attr("data-show", "false");
        $(wrapper).find(".back,.header_right").attr("data-show", "true");
      }
    }
  };
  ...
  */
  worf.prompt = {
    currentId: "",
    html: {
      //<span class="dd_l_icon icon"></span>
      base: '<div class="dd_layer_wrap" id="{id}" style="display:block;"><div class="dd_layer"><div class="dd_l_content"><div class="dd_l_cell"><div class="dd_l_title hide">{title}</div><div class="dd_l_text">{content}</div></div></div><div class="dd_l_btn">{buttons}</div></div>',
      tip: "<div id=\"promptTipBox\"><div class=\"tip-bg\" ></div><div class=\"tip-box\"><div class=\"cell\"><div class=\"tip-icon\"></div><p class=\"tip-title\"></p><p class=\"tip-message\"></p></div></div></div>"
    },
    _init: function (options) {
      var wrapper = $(options.id);
      if (wrapper.length == 0) {
        var html = worf.prompt.html.base.replace("{id}", options.id.replace("#", "")).replace("{title}", options.title).replace("{content}", options.message);
        $("body").append(html.replace("{buttons}", options.btns));
        options.title && $(options.id).find(".dd_l_title").removeClass("hide");
        if (options.icon) {
          $(options.id).find(".dd_l_content").addClass("show-icon").find(".icon").attr("class", "dd_l_icon icon").addClass(options.icon);
        }
        wrapper = $(options.id);
      } else {
        var $prompt = wrapper;
        $prompt.find(".dd_l_text").html(options.message);
        var title = $prompt.find(".dd_l_title").addClass("hide");
        options.title && title.html(options.title).removeClass("hide");
        options.icon && $prompt.find(".dd_l_content").addClass("show-icon").find(".icon").attr("class", "dd_l_icon icon").addClass(options.icon);
        $prompt.find(".dd_l_btn").html(options.btns);
        $prompt.show();
      }
      if (options.icon == "-1") {
        wrapper.find(".dd_l_icon").remove();
      }
      worf.prompt.isShow = true;
    },
    /*确定，取消*/
    confirm: function (title, okFunc, cancelFunc, options) {
      options = options || {};
      //关闭已打开的
      worf.prompt.closeMe();
      var currentId = worf.prompt.currentId = "#promptConfirmBox";
      var btnHtml = '<div class="sure" id="btnPromptOK">{sure}</div><div class="cancle" id="btnPromptCancel">{cancel}</div>';
      btnHtml = btnHtml.replace("{sure}", options.okText || "确定").replace("{cancel}", options.cancelText || "取消");
      worf.prompt._init({ id: currentId, message: options.message || "", title: title, btns: btnHtml });
      $("#btnPromptOK").off("click").on("click", okFunc || worf.prompt.closeMe);
      $("#btnPromptCancel").off("click").on("click", cancelFunc || worf.prompt.closeMe);
    },
    alert: function (title, okFunc, options) {
      options = options || {};
      //关闭已打开的
      worf.prompt.closeMe();
      var currentId = worf.prompt.currentId = "#promptAlertBox";
      var btnHtml = '<div class="{class}" id="btnAlertOK">{text}</div>';
      btnHtml = btnHtml.replace("{text}", options.okText || "确定").replace("{class}", options.cancelText || "long");
      worf.prompt._init({ id: currentId, title: title, message: options.message || "", btns: btnHtml, icon: options.icon || "icon-fine" });
      $("#btnAlertOK").off("click").on("click", okFunc || worf.prompt.closeMe);
    },
    /*成功*/
    success: function (title, okFunc, options) {
      options = options || {};
      options.icon = "icon-fine";
      worf.prompt.alert(title, okFunc, options);
    },
    /*失败*/
    fail: function (title, okFunc, options) {
      options = options || {};
    ...
    /*
    * tip
    * @param options: {message:"detail message",time:1000,icon:"none/success/fail"}
    */
    tip: function (title, options, callback) {
      if (!title) return;
      //参数调整
      ...
      //关闭已打开的
      worf.prompt.closeMe();
      options = $.extend({ time: 2000, icon: "none", message: "" }, options || {});
      var currentId = worf.prompt.currentId = "#promptTipBox";
      if ($(currentId).length == 0) {
        $("body").append(worf.prompt.html.tip);
      }
      else {
        $(currentId).replaceWith(worf.prompt.html.tip);
      }
      var $current = $(currentId);
      $current.find(".tip-title").html(title || "未知异常");
      $current.find(".tip-message").html(options.message);
      if (options.icon != "none") {
        var iconClass = { "success": "icon icon-success", "error": "icon icon-error" }[options.icon];
        $current.find(".tip-icon").html('<i class="' + iconClass + '"></i>').show();
      }
      $current.css("display", "table");
      worf.prompt.isShow = true;
      clearTimeout(window.pomptTipTimer);
      window.pomptTipTimer = setTimeout(function () {
        worf.prompt.closeMe();
        callback && callback();
      }, options.time);
    },
    closeMe: function () {
      worf.prompt.isShow = false;
      $(worf.prompt.currentId).hide();
    }
  };
   ...
   */
  var ajaxFlag = {};
  worf.ajax = function (opt) {
    if (ajaxFlag[opt.url]) return;
    
    ajaxFlag[opt.url] = true;//防止重复ajax
    
    //可以连续多次ajax请求
    if(opt.moreAjax == true){
    	ajaxFlag[opt.url] = false;
    }
    
    if (opt.animate == true) {
      var overlayTip = opt.overlayTip || "提交中...";
      worf.ajax.overlayShow(overlayTip);
    }
    opt.data = opt.data || {};
    ...
    $.ajax({
      ...
      success: function (json) {
        delete (ajaxFlag[opt.url]);
        /*
         * 1-成功；1000-无效的登录；
         * 1001-此账号已在别处登录，请重新登录；
         * 1002-此账号已在别处登录登录五次，请重新登录；
         * 1003-该用户名不存在；
         * 1004-您的账户已被停用；
         * 1005-用户名或密码错误；
         * 9001-参数错误；
         * 9999-操作失败，请稍后再试
         */
        if(json.status==1 || json.status==9001 || json.status==9999){
        	opt.success && opt.success(json);
        }else{
          if(opt.errorGoLogin){
        		worf.prompt.tip(json.message);
        		setTimeout(function(){
	        		worf.nav.replace(worf.origin+"/view/login.html?state="+location.href);
	        	},1000);
        	}else{
        		opt.success && opt.success(json);
        	}
      ...
      error: function (err) {
        ...
        var message = "请检查您的网络连接是否正常";
        opt.errorTip && opt.errorTip({ status: -10000, message: message }) || worf.prompt.tip(message);
        worf.ajax.overlayHide();
      }
    });
  };
  worf.ajax.overlayShow = function (text) {
    var overlay = $("#overlay");
    if (overlay.length > 0) {
      overlay.show();
    } else {
      $("body").append('<div class="overlay-bg"  id="overlay"><div class="overlay"><div class="spinner"><div class="spinner-container container1"><div class="circle1"></div><div class="circle2"></div><div class="circle3"></div><div class="circle4"></div></div><div class="spinner-container container2"><div class="circle1"></div><div class="circle2"></div><div class="circle3"></div><div class="circle4"></div></div><div class="sp-text">LOADING</div></div></div>');
      //text = text || "加载中...";
      //$("body").append('<div class="overlay-bg"  id="overlay"><div class="overlay"><div class="overlay-coin-wrap"><span class="overlay-coin"></span></div><span class="overlay-text">' + text + '</span></div></div>');
    }
    worf.ajax.isLoading = true;
  }
  worf.ajax.overlayHide = function () {
    worf.ajax.isLoading = false;
    $("#overlay").hide();
  }
  worf.ajax.SUCCESS = "success";
  /*
  * user
  */
  worf.user = {
    setOpenId: function(id) {
      worf.cookie.set("openid",id);
    },
    getOpenId: function () {
      var openid = worf.cookie.get("openid");
      var defaultId ="";
      if(worf.debug && !openid){
      		defaultId = new Date().getTime();
					worf.cookie.set("openid", defaultId);
      }
      return  openid || defaultId;
    },
    setSessionToken:function(session_token){
    	worf.cookie.set("session_token",session_token);
    },
    getSessionToken:function(){
    	var session_token = worf.cookie.get("session_token");
    	if(session_token){
  ...
  */
  worf.tools = {
    ...
    formatMoney: function (money) {
      money = (money || "0").toString().replace(/[^\.\d]/ig,"");
      money = parseFloat(money, 10).toFixed(2);
      money = money.split("").reverse().join("").replace(/(\d{3})/ig, "$1,");
      money = money.split("").reverse().join("").replace(/^\,/, "");
      return money;
    },
    /*检查手机号码格式*/
    ...
    phoneFormat: function (num) {
      var data = (num || "").split("");
      if (data.length > 3) data.splice(3, 0, " ");
      if (data.length > 5) data.splice(8, 0, " ");
      return data.join("");
    },
    /*手机隐藏中间四位*/
    phoneEncrypt: function (num) {
      if (num.length == 11) {
        return num.substr(0, 4) + "****" + num.substr(7, 4);
      }
      return num;
    },
    /*倒计时*/
    countdown: function (options) {
      options = $.extend({
        id: worf.tools.getTimestamp(),
        max: 60,
        interval: 1000,
        before: function () { console.log("before"); },
        after: function () { console.log("after"); },
        jump: function (time) { console.log(time); }
      }, options || {});
      options.before();
      var time = options.max;
      window.clearTimeout(window.countdownTimer);
      (function jump() {
        if (time == 0) return options.after();
        options.jump(time);
         time--;
        window.countdownTimer = window.setTimeout(function () { jump(); }, options.interval);
      })();
	  /*重置*/
      var reset =function(){
      		window.clearTimeout(window.countdownTimer);
    ...
    dateFormat: function (timestamp, format) {
      ...
      var newTime = new Date(Number(timestamp));
      return newTime.Format(format || "yyyy/MM/dd");
    },
    /*获取url参数*/
    ...
    * 限制提交频率
    */
    requestLimit: function () {
      return (worf.prompt.isShow || worf.ajax.isLoading);
    },
    /*
    * 修复安卓数字键盘无法输入小数
    * (输入框的类型从number改为tel）
    * (不能一律改为tel，因为tel在ios上没有小数点键）
    **/
    fixAndrodNumber: function () {
      if (worf.device.android) {
        $("input[type='number']").attr("type", "tel");
      }

      $("input[type='number'],input[type='tel']").off("input").on("input", function () {
        var me = $(this);
        var maxLength = me.attr("maxlength") || 12;
        var dotLength = me.attr("dot");
        var value = worf.tools.val(this) || "";
        var fixValue = worf.tools.fixNumber(value, maxLength, dotLength);
        if (fixValue != value) {
          this.value = fixValue;
        }
    ...
    * 修复数字
    */
    fixNumber: function (value, maxLength, dotLength, fixNullDot) {
      //字符修复
      ...
      //    nums[0] = "0";
      //}
      if (dotLength == 0 && nums.length > 1) {
        //小数部分为空 直接删掉
        nums.length = 1;
      }
      if (dotLength && nums.length > 1 && nums[1].length > dotLength) {
        nums[1] = nums[1].slice(0, dotLength);
      }
      //长度修复
      value = nums.join(".");
   ...
   */
  worf.animate = {
  	_bindEvent:function(el,callback){
  		callback && $(el).one('transitionend webkitAnimationEnd',callback);
  	},
  	/*左滑进场*/
    sliderLeft: function (el) {
      this._bindEvent(el,function () {
        	$(el).attr("style", "-webkit-transform:inherit");
      });
      $(el).removeClass("slide_hidden").addClass("slide_visible");
    },
    /*右滑出场*/
    sliderRight: function (el) {
      ...
      $(el).removeClass("slide_visible").css({ "display": "block" }).addClass("slide_hidden");
    },
    /*淡入*/
    fadeIn:function(el,callback){
    	var $el = $(el);
    	this._bindEvent($el,function(){
    		$el.removeClass("worf-ani-fadein");
    		callback && callback();
    	});
    	$el.addClass("worf-ani-fadein");
    },
    /*淡出*/
    fadeOut:function(el,callback){
    	var $el = $(el);
    	this._bindEvent($el,function(){
    		$el.removeClass("worf-ani-fadeout");
    		callback && callback();
    	});
    	$el.addClass("worf-ani-fadeout");
    },
    /*放大*/
    zoomIn:function(el,callback){
    	var $el = $(el);
    	this._bindEvent($el,function(){
    		$el.removeClass("worf-ani-zoomin");
    		callback && callback();
    	});
      	$el.addClass("worf-ani-zoomin");
    },
    /*缩小*/
    zoomOut:function(el,callback){
    	var $el = $(el);
    	this._bindEvent($el,function(){
    		$el.removeClass("worf-ani-zoomout");
    		callback && callback();
    	});
	   $el.addClass("worf-ani-zoomout");
    }
  };
   ...
   * @param form 验证的对象
   */
  worf.validate = function (form, options) {
    options = options || {};
    var $form = $(form);
    var isOK = true;
    var value = "";
    var valid = function (el) {
      ...
      if (that.is("input")) {
        ...
        value = that.attr("data-value") || that.data("value");
      }
      if (value !== 0 && !value) {
        value = "";
      }
      ...
      var lableName = label.data("for") ? label.data("for") : (label.html() || "").replace(/[\:：\*]/ig, "") || "！未知！";
      //必填验证
      var requiredMessage = that.data("valRequiredMs");
      if (rule.indexOf("required") > -1 && value == "") {
        worf.prompt.tip(requiredMessage || ("请输入" + lableName));
        isOK = false;
        return false;
      }
      //非必填
      ...
      //正则验证
      var regex = that.data("valRegex");
      var regexMessage = that.data("valRegexMs");
      if (regex && !new RegExp(regex, "ig").test(value)) {
        worf.prompt.tip(regexMessage || (lableName + "格式不正确"));
        isOK = false;
        return false;
      }
      return isOK;
    ...
    var data = {};
    $form.find(options.filter || "input[data-val]").each(function () {
      if (!valid(this)) return false;
      var that = $(this);
      if (!that.data("nosubmit")) {
        data[that.attr("name")] = $.trim(value);
      }
    });
  ...
  worf.form = {
    getData: function (form) {
      ...
      $(form).find("input,select,textarea").each(function () {
        var that = $(this); data[that.attr("name")] = $.trim(that.val() || that.text() || "");
      });
      return data;
    },
    bindData: function (form, data) {
      if (!data) return;
      var $form = $(form);
      for (var key in data) {
        $form.find("[name='" + key + "']").html(data[key]);
      }
    },
    /*重置*/
    reset: function (form, options) {
      options = options || {};
      $(form).find("input.form-value,span.form-value").each(function () {
        var that = $(this);
        if (that.is("input")) {
          that.data("value", "").val("");
        } else {
          that.data("value", "").text("");
        }
      });
  ...
  $(function () {
    ...
    $(".back").off("click").on("click", function () {
      if (this.dataset.special) {
        this.dataset.special != "true" && eval(this.dataset.special);
        return;
      }
      worf.nav.back();
    });
